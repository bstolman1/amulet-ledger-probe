    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          {[
            { label: "Last 24 Hours", count: newValidators.length, icon: Calendar, color: "text-primary" },
            {
              label: "Last 7 Days",
              count: weeklyValidators.length + newValidators.length,
              icon: TrendingUp,
              color: "text-chart-2",
            },
            {
              label: "Last 30 Days",
              count: monthlyValidators.length + weeklyValidators.length + newValidators.length,
              icon: Users,
              color: "text-chart-3",
            },
            {
            ...
            { label: "All Time", count: allTimeValidators.length, icon: Users, color: "text-primary" },
          ].map((stat, i) => (
            <Card key={i} className="glass-card">
              <div className="p-4">
                ...
                {validatorsLoading ? (
                  ...
                  <>
                    <p className={`text-3xl font-bold mb-1 ${stat.color}`}>{stat.count}</p>
                    <p className="text-xs text-muted-foreground">
                      {stat.label === "All Time" ? "Total validators" : "New validators"}
                    </p>
                  </>
                )}
              </div>
        ...
        {!usageLoading && !usageError && usageChartData && (
          <Card className="glass-card">
            <div className="p-6">
              <h3 className="text-xl font-bold mb-6">Network Usage Statistics (Last 90 Days)</h3>
              <ResponsiveContainer width="100%" height={350}>
                <AreaChart data={usageChartData}>
                  ...
                  <Area
                    type="monotone"
                    dataKey="dailyActiveParties"
                    stroke="#8884d8"
                    fill="#8884d8"
                    fillOpacity={0.3}
                  />
                  <Area type="monotone" dataKey="cumulativeParties" stroke="#82ca9d" fill="#82ca9d" fillOpacity={0.2} />
                  <Area type="monotone" dataKey="dailyTransactions" stroke="#ffc658" fill="#ffc658" fillOpacity={0.2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </Card>
