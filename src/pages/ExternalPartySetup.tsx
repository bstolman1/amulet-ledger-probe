...
const ExternalPartySetup = () => {
  ...
  const filteredProposals = proposalsData.filter((proposal: any) => {
    ...
    return party.toLowerCase().includes(searchTerm.toLowerCase()) ||
           requester.toLowerCase().includes(searchTerm.toLowerCase());
  }).slice(0, 100);

  const pendingCount = proposalsData.filter((p: any) => 
    (p.payload?.status || p.status || 'pending').toLowerCase() === 'pending'
  ).length;

  const approvedCount = proposalsData.filter((p: any) => 
    (p.payload?.status || p.status || 'pending').toLowerCase() === 'approved'
  ).length;
        ...
        <Card className="p-6">
          ...
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-8">
              No external party setup proposals found
            </p>
          ) : (
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <p className="text-sm font-medium mb-1">
                          External Party: {formatParty(externalParty)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Requester: {formatParty(requester)}
                        </p>
                      </div>
