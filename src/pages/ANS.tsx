...
const ANS = () => {
  ...
  const enrichedEntries = ansEntries.map((entry: any) => {
    const context = ansContexts.find((ctx: any) => ctx.payload?.name === entry.payload?.name || ctx.name === entry.payload?.name);
    return {
  ...
  const filteredEntries = enrichedEntries.filter((entry: any) =>
      ...
      <div className="space-y-6">
        ...
        <Card className="glass-card p-6">
          <div className="relative">
            ...
            <Input placeholder="Search ANS entries..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
          </div>
        </Card>
