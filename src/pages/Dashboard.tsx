...
const Dashboard = () => {
  ...
  const totalRewardBps = configData?.totalRewardBps ?? 0;
  const totalRewardPercent = (totalRewardBps / 10000).toFixed(2);
  ...
  useEffect(() => {
    if (configData) {
      console.log(`✅ Config parsed: ${totalSVs} SVs, totalRewardBps=${totalRewardBps}, ghosts=${ghostSVs}`);
    }
  }, [configData]);
          ...
          {configLoading ? (
            ...
            <>
              <div className="text-sm text-gray-400 space-y-1 mb-6">
                ...
                <p>
                  • Total Reward Weight (bps): <span className="text-gray-200">{totalRewardBps.toLocaleString()}</span>
                </p>
                <p>
                  • Total Reward Weight (%): <span className="text-gray-200">{totalRewardPercent}%</span>
                </p>
                <p>
                  • Last Updated:{" "}
              ...
              {/* SuperValidator Table */}
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-left border-collapse border border-gray-700">
                  <thead>
                    <tr className="bg-gray-800/50 text-gray-300">
                      <th className="px-3 py-2 border border-gray-700">SV Name</th>
                      <th className="px-3 py-2 border border-gray-700">Operator</th>
                      <th className="px-3 py-2 border border-gray-700 text-right">Weight (bps)</th>
                      <th className="px-3 py-2 border border-gray-700 text-right">Weight (%)</th>
                      <th className="px-3 py-2 border border-gray-700 text-center">Ghost</th>
                    </tr>
                  </thead>
                  <tbody>
                    {configData?.superValidators?.map((sv, i) => (
                      <tr key={i} className={`${sv.isGhost ? "bg-red-900/20" : "hover:bg-gray-800/40"}`}>
                        <td className="px-3 py-2 border border-gray-700 text-gray-100">{sv.name}</td>
                        <td className="px-3 py-2 border border-gray-700 text-gray-400">{sv.operatorName}</td>
                        <td className="px-3 py-2 border border-gray-700 text-right text-gray-200">
                          {sv.weight.toLocaleString()}
                        </td>
                        <td className="px-3 py-2 border border-gray-700 text-right text-gray-200">
                          {(sv.weight / 10000).toFixed(2)}%
                        </td>
                        <td className="px-3 py-2 border border-gray-700 text-center">{sv.isGhost ? "👻" : "✅"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
