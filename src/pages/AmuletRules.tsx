...
const normalizeIssuanceValue = (value: any): NormalizedIssuanceValue | undefined => {
  ...
  return {
    ...
    validatorRewardPercentage: pickFirstDefined(
      value.validatorRewardPercentage,
      value.validator_reward_percentage
    ),
    ...
    unfeaturedAppRewardCap: pickFirstDefined(
      value.unfeaturedAppRewardCap,
      value.unfeatured_app_reward_cap
    ),
    ...
    transferConfig: transferConfig
      ? {
          ...
          maxNumLockHolders: pickFirstDefined(
            transferConfig.maxNumLockHolders,
            transferConfig.max_num_lock_holders
          ),
        }
      : undefined,
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Live configuration values pulled from the latest ACS snapshot for templates ending in
            AmuletRules. This view updates automatically when new snapshots are available, so values
            stay current.
          </p>
        </div>
        ...
        {!isLoading && !hasData && (
          <Alert>
            ...
            <AlertDescription>
              We couldn't find any AmuletRules contracts in the latest snapshot. Trigger a new
              snapshot or verify that the template exists in the environment.
            </AlertDescription>
          </Alert>
        )}
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-2xl font-semibold">
                    {normalizedRule?.featuredAppActivityMarkerAmount || "—"}
                  </p>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">
                        Initial rate {transferConfig?.transferFee?.initialRate || "—"}
                      </Badge>
                    </div>
                    <Table>
                      <TableHeader>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div className="p-4 rounded-lg bg-muted/50 space-y-1">
                    ...
                    <p className="text-2xl font-semibold">
                      {issuanceCurve?.initialValue?.amuletToIssuePerYear || "—"}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg bg-muted/50 space-y-1">
                        ...
                        issuanceCurve?.futureValues?.map((future, index) => (
                          <TableRow key={`${future.effectiveAfterMicroseconds}-${index}`}>
                            ...
                            <TableCell className="text-right">
                              {future.values?.amuletToIssuePerYear || "—"}
                            </TableCell>
                            <TableCell className="text-right">
