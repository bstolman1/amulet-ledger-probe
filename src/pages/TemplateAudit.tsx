...
const TemplateRow = ({ snapshotId, templateId, contractCount }: { snapshotId: string; templateId: string; contractCount: number; }) => {
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <button className="inline-flex items-center justify-center" onClick={() => setOpen(!open)} aria-label={open ? "Collapse" : "Expand"}>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="text-xs">{contractCount} entries</Badge>
              </div>
              <div className="flex flex-wrap gap-2">
                {pages.length > 0 ? pages.map((p) => (
                  <Badge key={p} variant="outline" className="text-xs">Used on: {p}</Badge>
                )) : (
                  <Badge variant="destructive" className="text-xs">Not used on any page</Badge>
                )}
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {data?.data && data.data.length > 0 ? (
                  <>
                    <p className="text-xs text-muted-foreground">Showing first {Math.min(3, data.data.length)} of {data.data.length} records</p>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">Verify every template's data is discoverable and which pages consume it.</p>
          </div>
