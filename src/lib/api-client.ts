...
export const scanApi = {
  ...
  async fetchLatestRound(): Promise<GetRoundOfLatestDataResponse> {
    // Use "round-of-latest-data" so both round and effectiveAt are available.
    const controller = new AbortController();
  ...
  // Get total balance from latest round totals instead of a deprecated endpoint
  ...
  // Fetch governance proposals by scanning ACS for VoteRequest contracts
  async fetchGovernanceProposals(): Promise<any[]> {
    try {
      ...
      for (const ev of acs.created_events) {
        ...
        const payload = (ev as any).create_arguments || {};
      ...
      // Fallback: if nothing found, list recent SV onboardings as executed items
