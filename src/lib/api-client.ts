...
export const scanApi = {
  ...
  // Some pages call scanApi.fetchTopValidators—provide a wrapper for compatibility
  async fetchTopValidators(limit: number): Promise<TopValidatorsByFaucetsResponse> {
    return this.fetchTopValidatorsByFaucets(limit);
  },
  ...
  async fetchGovernanceProposals(): Promise<any[]> {
    try {
      ...
      // best-effort heuristic from ACS if needed in future:
      // const snap = await this.fetchAcsSnapshotTimestamp(latest.effectiveAt, 0);
      // const acs = await this.fetchStateAcs({...});

      const proposals: any[] = [];

      // Fallback: if nothing else, list recent SV onboardings as executed items
