...
let UPLOAD_CHUNK_SIZE = parseInt(process.env.UPLOAD_CHUNK_SIZE || "3", 10);
const UPLOAD_DELAY_MS = parseInt(process.env.UPLOAD_DELAY_MS || "500", 10);
const MIN_CHUNK_SIZE = 1;
const MAX_CHUNK_SIZE = parseInt(process.env.UPLOAD_CHUNK_SIZE || "3", 10);
...
console.log(
  `   - Edge Function URL: ${
    EDGE_FUNCTION_URL ? "✅ Configured" : "❌ Not configured"
  }`
);
console.log(
  `   - Webhook Secret: ${
    WEBHOOK_SECRET ? "✅ Configured" : "❌ Not configured"
  }`
);
console.log(
...
console.log(
  `   - Supabase Anon Key: ${
    SUPABASE_ANON_KEY ? "✅ Configured" : "❌ Not configured"
  }`
);
console.log(
  `   - Supabase Client: ${
...
console.log(
  `   - Page Size: ${parseInt(process.env.PAGE_SIZE || "500", 10)}`
);
console.log(
  `   - Upload Chunk Size: ${UPLOAD_CHUNK_SIZE} (min: ${MIN_CHUNK_SIZE}, max: ${MAX_CHUNK_SIZE}) - adaptive`
);
console.log(
...
console.log(
  `   - Max In-Flight Uploads: ${parseInt(
    process.env.MAX_INFLIGHT_UPLOADS || "1",
    10
  )}`
);
console.log("=".repeat(80) + "\n");
...
async function uploadToEdgeFunction(phase, data, retryCount = 0) {
  ...
  try {
    ...
    if (phase === "append" && UPLOAD_CHUNK_SIZE < MAX_CHUNK_SIZE) {
      ...
      console.log(
        `✅ Upload successful - increased chunk size to ${UPLOAD_CHUNK_SIZE}`
      );
    }
    ...
    console.error(
      `❌ Upload failed (${phase}): ${error.message} (status: ${
        status || "unknown"
      })`
    );
    ...
    if (is546Error(error) && phase === "append" && retryCount < MAX_RETRIES) {
      if (UPLOAD_CHUNK_SIZE > MIN_CHUNK_SIZE) {
        ...
        console.log(
          `⚠️  Reduced chunk size to ${UPLOAD_CHUNK_SIZE} due to 546 error`
        );
      }
...
async function checkForExistingSnapshot(migration_id) {
  if (!supabase) {
    ...
    console.log(
      "   This means a new snapshot will be created even if one is in progress!\n"
    );
    return null;
  }
...
async function fetchAllACS(baseUrl, migration_id, record_time, existingSnapshot) {
    ...
    while (retryCount < MAX_RETRIES && !success) {
      try {
        ...
        if (snapshotId && Object.keys(pendingUploads).length >= UPLOAD_CHUNK_SIZE) {
          ...
          for (const templateId of batchKeys) {
            ...
            if (entryCount > ENTRIES_PER_CHUNK) {
              console.log(
                `📦 Splitting ${templateId} (${entryCount} entries) into chunks of ${ENTRIES_PER_CHUNK}...`
              );
          ...
          console.log(
            `📤 Starting upload of ${chunkedTemplates.length} chunks from ${batchKeys.length} templates (chunk size: ${UPLOAD_CHUNK_SIZE})...`
          );
          ...
          const uploadPromise = (async () => {
            try {
              ...
              console.log(
                `⚠️  Re-queueing ${Object.keys(uploadSnapshot).length} templates for retry`
              );
        ...
        if (page % 10 === 0) {
          ...
          const pagesPerMin =
            elapsedMinutes > 0 ? (page / elapsedMinutes).toFixed(2) : 0;
          const eventsPerPage =
            page > 0 ? (allEvents.length / page).toFixed(0) : 0;
        ...
        if (isRetryable && retryCount < MAX_RETRIES - 1) {
          ...
          console.log(
            `🔄 Retry ${retryCount}/${MAX_RETRIES} in ${
              delay + jitter
            }ms (with jitter)...`
          );
  ...
  if (inflightUploads.length > 0) {
    console.log(
      `⏳ Waiting for ${inflightUploads.length} in-flight uploads...`
    );
  ...
  if (snapshotId) {
    ...
    console.log(
      "⚠️ No upload configured (missing EDGE_FUNCTION_URL or ACS_UPLOAD_WEBHOOK_SECRET)"
    );
  }
...
async function run() {
  try {
    ...
    if (existingSnapshot) {
      ...
      console.log("=".repeat(80));
      console.log(
        "   This GitHub Actions run is continuing a previous snapshot."
      );
      console.log(
        "   This is the expected behavior when a snapshot takes longer than 2 hours."
      );
      console.log(`   - Continuing snapshot: ${existingSnapshot.id}`);
      console.log(
        `   - Will resume from page: ${existingSnapshot.processed_pages || 1}`
      );
      console.log(
        `   - Will resume from cursor: ${existingSnapshot.cursor_after || 0}`
      );
    } else {
      console.log("🆕 DECISION: STARTING NEW SNAPSHOT");
      console.log("=".repeat(80));
      console.log(
        "   No in-progress snapshot found. Creating a new one for the current migration."
      );
      console.log(`   - Migration ID: ${migration_id}`);
