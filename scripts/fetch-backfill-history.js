...
async function fetchMigrationInfo(migration_id) {
  try {
    ...
    if (err.response && err.response.status === 404) {
      console.log(
        `ℹ️  No backfilling info for migration_id=${migration_id} (404)`,
      );
      return null;
    }
    throw err;
...
async function updateCursorLastBefore(
  migration_id,
  synchronizer_id,
  last_before,
  complete = false,
...
async function backfillForSynchronizer(migration_id, range) {
  ...
  while (true) {
    console.log(
      `   ➜ Requesting /backfilling/updates-before before=${before}, at_or_after=${atOrAfter}`,
    );
    ...
    try {
      ...
      console.error(
        `   ❌ Error from updates-before (status ${status || "n/a"}): ${msg}`,
      );
    ...
    if (!earliest || earliest <= atOrAfter) {
      console.log(
        "   ✅ Reached lower bound of range; marking synchronizer as complete.",
      );
    ...
    console.log(
      `   📥 Stored ${txs.length} updates, new before=${before} (still > ${atOrAfter})`,
    );
  }
}
...
async function run() {
  ...
  for (const migration_id of migrations) {
    ...
    console.log(
      `   Found ${ranges.length} synchronizer ranges for migration ${migration_id}`,
    );
    ...
    console.log(
      `✅ Completed migration ${migration_id} (all synchronizers backfilled as far as Scan allows).`,
    );
  }
