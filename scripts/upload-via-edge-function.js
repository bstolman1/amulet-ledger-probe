...
const MAX_CHUNK_SIZE = parseInt(
  process.env.MAX_UPLOAD_CHUNK_SIZE || String(CHUNK_SIZE),
  10
);
...
async function callEdgeFunction(phase, payload) {
  ...
  if (!edgeFunctionUrl || !webhookSecret) {
    throw new Error(
      "Missing EDGE_FUNCTION_URL or ACS_UPLOAD_WEBHOOK_SECRET environment variables"
    );
  }
  ...
  while (true) {
    try {
      ...
      if (
        ...
        console.log(
          `⚠️  Worker limit hit; reducing CHUNK_SIZE to ${CHUNK_SIZE}`
        );
      }
...
export async function uploadBatch({
  ...
  try {
    ...
    const templates = Object.entries(templatesData).map(
      ([templateId, data]) => ({
    ...
    for (let i = 0; i < templates.length; ) {
      ...
      try {
        ...
        const processed = typeof result?.processed === "number"
          ? result.processed
          : chunk.length;
        ...
        const maxChunkSizeGrowth = Math.max(
          initialConfiguredChunkSize,
          MAX_CHUNK_SIZE
        );
        ...
        console.error(
          `❌ Fatal error uploading chunk ${chunkNum}: ${err.message}`
        );
        throw err;
      }
    }
  ...
  (async () => {
    try {
      ...
      const summaryData = JSON.parse(
        fs.readFileSync("circulating-supply-single-sv.json", "utf-8")
      );
